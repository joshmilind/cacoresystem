...
public class TargetTest extends TestCase{
    ...
    private static Logger _logger = Logger.getLogger(TargetTest.class);
    
	public static final Class CLASS = TargetTest.class;
	...
	public static junit.framework.Test suite() {
		...
		//suite.addTest(new TargetTest("testPopulate"));
		suite.addTest(new TargetTest("testRetrieve"));
	...
	public void testPopulate(){
		try{
		        gov.nih.nci.cabio.domain.ws.Target target = new gov.nih.nci.cabio.domain.ws.Target() ;
			save(target);
		}catch(Exception ex){
		    ex.printStackTrace();
		    fail("Error retrieving Target. Got " + ex.getClass().getName() + ": " + ex.getMessage());
	...
	public void testRetrieve(){
		try{
			...
			sess.createCriteria(gov.nih.nci.cabio.domain.ws.Target.class).list();
			tx.commit();
	...
	((gov.nih.nci.cabio.domain.ws.Target)obj).setId(new java.lang.Long("1"));
